...
export function useTweaks<T extends Record<string, unknown>>(defaults: T): [T, <K extends keyof T>(keyOrEdits: K | Partial<T>, val?: T[K]) => void] {
...
export function TweaksPanel({ title = "Tweaks", noDeckControls = false, children }: TweaksPanelProps) {
  ...
  const [railEnabled, setRailEnabled] = useState(
    () => hasDeckStage && !!(document.querySelector("deck-stage") as { _railEnabled?: boolean } | null)?._railEnabled,
  );
  useEffect(() => {
...
export function TweakSection(props: { title?: string; label?: string; children?: ReactNode }) {
  const heading = props.title ?? props.label;
  return (
    <>
      {heading != null && heading !== "" && <div className="twk-sect">{heading}</div>}
      {props.children}
    </>
  );
}
...
type RadioOpt<T extends string | number | boolean = string> = T | { value: T; label: ReactNode };

interface TweakRadioProps<T extends string | number | boolean> {
  label: ReactNode;
  value: T;
  options: RadioOpt<T>[];
  onChange: (v: T) => void;
...
export function TweakRadio<T extends string | number | boolean>({ label, value, options, onChange }: TweakRadioProps<T>) {
  ...
  const labelLen = (o: RadioOpt<T>) => String(typeof o === "object" && o !== null && "label" in o ? o.label : o).length;
  const maxLen = options.reduce((m, o) => Math.max(m, labelLen(o)), 0);
  const fitsAsSegments = maxLen <= ({ 2: 16, 3: 10 }[options.length] ?? 0);
  if (!fitsAsSegments) {
    const resolve = (s: string) => {
      const m = options.find((o) => String(typeof o === "object" && o !== null && "value" in o ? o.value : o) === s);
      if (m === undefined) return s as unknown as T;
      return typeof m === "object" && m !== null && "value" in m ? m.value : (m as T);
    };
    return (
      <TweakSelect
        label={label}
  ...
  const opts = options.map((o) => (typeof o === "object" && o !== null && "value" in o ? o : { value: o as T, label: String(o) }));
...
interface TweakSelectProps {
  label: ReactNode;
  value: string;
  options: SelOpt[] | RadioOpt<string>[];
  ...
  return (
    <TweakRow label={label}>
      <select className="twk-field" value={value} onChange={(e) => onChange(e.target.value)}>
        {options.map((o) => {
          const v = typeof o === "object" && o !== null && "value" in o ? String(o.value) : String(o);
          const l = typeof o === "object" && o !== null && "label" in o ? o.label : String(o);
          return (
...
export function TweakColor({ label, value, options, onChange }: TweakColorProps) {
  if (!options || !options.length) {
    const v = typeof value === "string" ? value : value[0] ?? "#000000";
    return (
      <div className="twk-row twk-row-h">
        <div className="twk-lbl">
          <span>{label}</span>
        </div>
        <input type="color" className="twk-swatch" value={v} onChange={(e) => onChange(e.target.value)} />
      </div>
    );
  }
  const key = (o: ColorOpt) => String(JSON.stringify(o)).toLowerCase();
  const cur = key(value);
  return (
    <TweakRow label={label}>
