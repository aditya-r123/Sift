...
## 8. Database schema — CLI migrations

This repo uses the Supabase CLI so schema changes (new tables, RLS policies) live as
versioned `.sql` files in `supabase/migrations/` — committed alongside feature code,
no dashboard clicks.

One-time: link the repo to your Supabase project:

```
npx supabase login                                    # opens browser, paste the token
npx supabase link --project-ref pvmowvztpcwriydcjdkq   # prompts for the DB password
```

`login` stores an access token on your machine. `link` records the project in
`supabase/config.toml`. The DB password is the one set when the project was created
in step 1 — reset it under **Project Settings** → **Database** if lost.
...
The first migration, `supabase/migrations/20260515101210_init_profiles.sql`, creates the
`profiles` table, its row-level-security policies, and a trigger that auto-inserts a
profile row whenever someone signs up (email/password or Google).

Adding a feature later (e.g. friends) is then just:

```
npm run db:new friends     # creates supabase/migrations/<timestamp>_friends.sql
                           # write the CREATE TABLE + RLS policies into that file
npm run db:push            # applies it
```

So every feature = app code + one migration file + `npm run db:push`. No new API keys,
no dashboard settings.

> `db:push` writes to your live database — review the SQL first. It only runs
> migrations not yet applied, so re-running it is safe.
...
- **Added** the Supabase CLI (`supabase` dev dependency), a `supabase/` scaffold, and the
  first migration under `supabase/migrations/`. New `db:new` / `db:push` npm scripts.
