-- Replaces the one-way follow model with a bidirectional request/accept system.
-- requester_id sends the invite; addressee_id can accept or decline.
-- Either side can delete the row to cancel, decline, or unfriend.
...
-- Only the requester can open a request; status defaults to 'pending'.
...
-- Dashboard-only view joining friends to emails and display names.
-- Not exposed via the PostgREST API (access revoked below).
